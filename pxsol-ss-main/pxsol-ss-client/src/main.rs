...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Config {
            ...
            if !program_id.is_none() || !network.is_none() {
                let _ = config_set(program_id, network)?;
            }
        }
    }
    Ok(())
...
fn get_config_file_path() -> std::path::PathBuf {
    ...
    return home_path.join(CONFIG_FILE_PATH);
}

fn get_current_config() -> Result<()> {
...
fn config_set(program_id: Option<String>, network: Option<String>) -> Result<()> {
    let path = get_config_file_path();

    // 设置默认配置
    let mut config_json: ConfigJson = ConfigJson::new();
    ...
    if !path.exists() {
        ...
        let reader = BufReader::new(file);
        config_json = serde_json::from_reader(reader)?;
    }
